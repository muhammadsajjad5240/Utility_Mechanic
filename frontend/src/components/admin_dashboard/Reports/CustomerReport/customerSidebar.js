    ...
    <div>
      ...
      <List>
        {[
         ...
         <Link to={"/mechanic-report?id=" + qurryString.parse(window.location.search).id+ "&User=" + qurryString.parse(window.location.search).User}><li title='Mechanic Report' style={{textAlign:'left', marginLeft:'25%'}}>Mechanic</li></Link>,
         <Link to={"/service-report?id=" + qurryString.parse(window.location.search).id+ "&User=" + qurryString.parse(window.location.search).User}><li title='Service Report' style={{textAlign:'left', marginLeft:'25%'}}>Service</li></Link>,
         <Link to={"/customer-report?id=" + qurryString.parse(window.location.search).id+ "&User=" + qurryString.parse(window.location.search).User}><li title='Customer Report' style={{textAlign:'left', marginLeft:'25%'}}>Customer</li></Link>,
         <Link to={"/order-report?id=" + qurryString.parse(window.location.search).id+ "&User=" + qurryString.parse(window.location.search).User}><li title='Order Report' style={{textAlign:'left', marginLeft:'25%'}}>Order</li></Link>,         
        ].map((text, index) => (
          <ListItem button key={text}>
            <ListItemText primary={text} />
          </ListItem>
    ...
    <div className={classes.root}>
      ...
      <AppBar position="fixed" className={classes.appBar}>
        <Toolbar>
          ...
          <center><h2 className='appBarHeading'>Admin Panle</h2></center>
          <React.Fragment>
      ...
      <main className={classes.content}>
        <div className={classes.toolbar} />
        <div className='coustomerreportdiv'>
        <CustomerReport />
        </div>
      </main>
    </div>
