      ...
      axios.post("/Admin/update-Admin", {
        ...
        .then(responce => {
          ...
          if (a === 'OK') {
            ...
            window.location.assign("/admin-profile?id=" + qurryString.parse(window.location.search).id+ "&User=" + qurryString.parse(window.location.search).User);
          }
        }
        ...
        <Modal backdrop='static' show={this.state.show} onHide={this.handleClose} className='modal-style'>
          ...
          <Modal.Body className='sign-modal-style'>
            <Form className='signInFormStyle'>
              ...
              <Row>
                <Col xs={12} lg={{ span: 10, offset: 1 }}>
                  <Link to={"/admin-profile?id=" + qurryString.parse(window.location.search).id+ "&User=" + qurryString.parse(window.location.search).User} style={{ color: 'White' }} ><Button variant='danger' className='signInBtn'>
                    Close</Button>
                  </Link>
                </Col>
              </Row>
